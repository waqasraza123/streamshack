...
class EventCheckoutController extends Controller
{
    ...
    public function postValidateTickets(Request $request, $event_id)
    {
        ...
        //dd(session()->get('ticket_order_'.$event->id));
    ...
    public function postValidateOrder(Request $request, $event_id)
    {
        ...
        $ticket_order['request_data'] = $request_data;
        ...
        $data = array(
            ...
            'notify_url' => url(route('postCreateOrder', ['event_id' => $event_id])),
            // Buyer details
            'name_first' => 'First Name',
            'name_last'  => 'Last Name',
            'email_address'=> $request->input('order_email'),
            // Transaction details
        ...
        Log:info("inserting data in temp_session with below signature");



        DB::table('session_temp')->insert([
            'signature' => $signature,
            'data' => json_encode($ticket_order)
        ]);
    ...
    public function postCreateOrder(Request $request, $event_id)
    {
        ...
        $ticket_order = json_decode(DB::table('session_temp')->where('signature', $payFastData['custom_str1'])->first()->data);
        ...
        try {
            ...
            $order_email = $payFastData['email_address'];
    ...
    public function registerUserDuringPayment($request){
        ...
        $request_data['first_name'] = $request_data['name_first'];
        $request_data['last_name'] = $request_data['name_last'];
        $request_data['email'] = $request_data['email_address'];


        $account_data = ['first_name' => $request_data['first_name'], 'last_name' => $request_data['last_name'], 'email' => $request_data['email']];
        ...
        $user_data = ['first_name' => $request_data['first_name'], 'last_name' => $request_data['last_name'], 'email' => $request_data['email']];
        ...
        if(!User::whereEmail($request_data['email'])->first()){

            $user_data['password'] = Hash::make($request_data['password']);
        ...
        if ($is_attendize) {
            ...
            Mail::send('Emails.ConfirmEmail',
                ...
                function ($message) use ($request_data) {
                    $message->to($request_data['email'], $request_data['first_name'])
                        ->subject(trans("Email.attendize_register"));
                });
        }

        Log:info("User Found");
        Log::info(User::whereEmail($request_data['email'])->first());
    ...
    public function receivePayFastData(){
    ...
    public function completeOrder($event_id, $return_json = true)
    {
        ...
        $payFastData = $this->receivePayFastData();
        $event_id = $payFastData['custom_int1'];
        $paymentStatus = $payFastData['payment_status'];
        $orderInformationDB = json_decode(DB::table('session_temp')->where('signature', $payFastData['custom_str1'])->first()->data);
        ...
        //try {
            ...
            //$ticket_questions = isset($orderInformationDB['ticket_holder_questions']) ? $orderInformationDB['ticket_holder_questions'] : [];


            /**
             * register the user
             */
            $newUserId = $this->registerUserDuringPayment($payFastData);
             ...
             * Create the order
             */
            if (isset($payFastData['transaction_id'])) {
                $order->transaction_id = $payFastData['transaction_id'][0];
            }

            if (isset($payFastData['transaction_data'][0]['payment_intent'])) {
                $order->payment_intent = $payFastData['transaction_data'][0]['payment_intent'];
            }


            if ($orderInformationDB->order_requires_payment && !isset($orderInformationDB->pay_offline)) {
                $order->payment_gateway_id = $orderInformationDB->payment_gateway->id;
            }
            $order->first_name = sanitise($payFastData['name_first']);
            $order->last_name = sanitise($payFastData['name_last']);
            $order->email = sanitise($payFastData['email_address']);
            $order->order_status_id = isset($orderInformationDB->pay_offline) ? config('attendize.order.awaiting_payment') : config('attendize.order.complete');
            $order->amount = $payFastData['amount_net'];
            $order->booking_fee = $payFastData['amount_fee'];
            $order->organiser_booking_fee = 0;
            $order->discount = 0.00;
            $order->account_id = $event->account->id;
            $order->event_id = $payFastData['custom_int1'];
            $order->is_payment_received = $paymentStatus == 'complete' ? 1 : 0;
            $order->purchaser_id = $newUserId;

            // Business details is selected, we need to save the business details
            if (isset($payFastData['is_business']) && (bool)$payFastData['is_business']) {
                $order->is_business = $payFastData['is_business'];
                $order->business_name = sanitise($payFastData['business_name']);
                $order->business_tax_number = sanitise($payFastData['business_tax_number']);
                $order->business_address_line_one = sanitise($payFastData['business_address_line1']);
                $order->business_address_line_two  = sanitise($payFastData['business_address_line2']);
                $order->business_address_state_province  = sanitise($payFastData['business_address_state']);
                $order->business_address_city = sanitise($payFastData['business_address_city']);
                $order->business_address_code = sanitise($payFastData['business_address_code']);

            }

            // Calculating grand total including tax
            //$orderService = new OrderService($ticket_order['order_total'], $ticket_order['total_booking_fee'], $event);
            $orderService = new OrderService($payFastData['amount_net'], 0, $event);
            $orderService->calculateFinalCosts();
            ...
            collect($request_data->tickets)->map(function($ticketDetail) use ($order) {
             ...
             */
            if (isset($request_data->affiliate_referral)) {
                $affiliate = Affiliate::where('name', '=', $request_data->affiliate_referral)
                    ->where('event_id', '=', $event_id)->first();
                $affiliate->increment('sales_volume', $order->amount + $order->organiser_booking_fee);
                $affiliate->increment('tickets_sold', $request_data['total_ticket_quantity']);
            }
            ...
            $event_stats->increment('tickets_sold', $request_data->total_ticket_quantity);

            if ($request_data->order_requires_payment) {
             ...
             * Add the attendees
             */
            foreach ($request_data->tickets as $attendee_details) {
                /*
                ...
                $ticket->increment('sales_volume', ($attendee_details->price * $attendee_details->qty));
                ...
                for ($i = 0; $i < $attendee_details->qty; $i++) {
                    ...
                    $attendee->first_name = strip_tags($request_data->request_data->ticket_holder_first_name[$i]->$ticketId);
                    $attendee->last_name = strip_tags($request_data->request_data->ticket_holder_last_name[$i]->$ticketId);
                    $attendee->email = $request_data->request_data->ticket_holder_email[$i]->$ticketId;
                    $attendee->event_id = $event_id;
                    $attendee->order_id = $order->id;
                    $attendee->ticket_id = $attendee_details->ticket->id;
                    $attendee->account_id = $event->account->id;
                    $attendee->reference_index = $attendee_increment;
                    $attendee->attendee_id = $newUserId;
                    $attendee->save();
